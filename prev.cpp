...
{ // This is called just once, at the start of V-REP.
    ...
    simEnableEventCallback(sim_message_eventcallback_openglcameraview,"BubbleRob",-1);
...
{ // This is called quite often. Just watch out for messages/events you want to handle
    ...
    {
        right=(int)((unsigned char*)customData)[0];
    }
        ...
        if ( (customData==NULL)||(std::string("BubbleRob").compare((char*)customData)==0) ) // is the command also meant for BubbleRob?
        {
            ...
            for (unsigned int i=0;i<allBubbleRobs.size();i++)
            {
                ...
                { // movement mode
                    ...
                    { // We move backward
                        simSetJointTargetVelocity(allBubbleRobs[i].motorHandles[0],right * (-3.1415f)*allBubbleRobs[i].backRelativeVelocities[0]);
                    ...
                    { // We move forward
                        simSetJointTargetVelocity(allBubbleRobs[i].motorHandles[0],right * 3.1415f);
